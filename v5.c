...
static inline char *parse_pointer(int *dest, char *s) {
...
void print_json(struct result *results, int nresults) {
    ...
    for (int i = 0; i < nresults; i++) {
        b += sprintf(b, "%s=%.1f/%.1f/%.1f%s", results[i].city,
                (float)results[i].min/10.0,
                (float)results[i].sum / (float)results[i].count / 10.0,
                (float)results[i].max/10.0, i < nresults - 1 ? ", " : "}");
    }
    *b = 0x0;
    puts(buf);
}
    ...
    while(1) {
        ...
        while(pos < &buffer[ret]) {
            ...
            if(c < 0) {
                ...
                bundle->results[c].min = min(curr, bundle->results[c].min);
                bundle->results[c].max = max(curr, bundle->results[c].max);
